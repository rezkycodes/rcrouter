...
- The first protocol-fidelity slice is complete (`08c5211f`); the compatibility matrix tracks four remaining bounded-loss cases, with nameless Responses calls, Claude tool-result images, redacted-thinking diagnostics, and unsupported-audio diagnostics now resolved. Cursor AgentService protobuf, CommandCode NDJSON, and Kiro EventStream executor coverage now pass as focused suites.
