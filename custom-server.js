...
http.createServer = (...args) => {
  ...
  const wrapped = (req, res) => {
    ...
    delete req.headers["x-rc-real-ip"];
    delete req.headers["x-forwarded-for"];
    delete req.headers["x-9r-via-proxy"];
    delete req.headers["x-rc-via-proxy"];
    delete req.headers["x-9r-peer-token"];
    delete req.headers["x-rc-peer-token"];
    req.headers["x-9r-real-ip"] = ip;
    req.headers["x-rc-real-ip"] = ip;
    req.headers["x-9r-peer-token"] = PEER_TOKEN;
    req.headers["x-rc-peer-token"] = PEER_TOKEN;
    if (viaProxy) {
      req.headers["x-9r-via-proxy"] = "1";
      req.headers["x-rc-via-proxy"] = "1";
    }
