#!/usr/bin/env node
// Cross-platform production build for the RcRouter app.
//
...
// The DB lives at DATA_DIR (env or ~/.rcrouter / ~/.9router) — outside .next/ so `next build`
...
function backupProductionDb() {
  ...
  const primaryDataDir = process.env.DATA_DIR || dotenv.DATA_DIR || path.join(require("os").homedir(), ".rcrouter");
  let dbFile = path.join(primaryDataDir, "db", "data.sqlite");
  if (!fs.existsSync(dbFile)) {
    // Check legacy .9router path too
    const legacyDb = path.join(require("os").homedir(), ".9router", "db", "data.sqlite");
  ...
  const backupsDir = path.join(path.dirname(dbFile), "backups");
...
const lintScript = path.join(appDir, "scripts", "lint-undef.cjs");
if (fs.existsSync(lintScript)) {
  ...
  try {
    ...
    if (e.status !== 0) {
      console.error("❌ no-undef lint FAILED — fix undefined variables before building");
...
// Also copy custom-server.js if it exists in root
if (fs.existsSync(path.join(appDir, "custom-server.js"))) {
  fs.cpSync(path.join(appDir, "custom-server.js"), path.join(appDir, distDir, "standalone", "custom-server.js"), { force: true });
  console.log(`▶ copied custom-server.js into ${distDir}/standalone/custom-server.js`);
}

fixStandaloneSymlinks(path.resolve(__dirname, "..", distDir, "standalone"));
...
if (fs.existsSync(srcLibDir)) {
  ...
  const shimContent = `// Auto-generated by build.js — resolves webpackIgnore import in instrumentation hook\n` +
    `export {\n` +
    `  getSettings, updateSettings, isCloudEnabled, getCloudUrl, exportSettings, invalidateSettingsCache,\n` +
