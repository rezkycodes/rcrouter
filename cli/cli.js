...
const PROCESS_IDENTIFIERS = [
  'rcrouter',
  '9router'  // Support cleanup of legacy processes
];
...
function getAppDataDir() {
  const target = process.platform === "win32"
    ? path.join(process.env.APPDATA || "", "rcrouter")
    : path.join(os.homedir(), ".rcrouter");

  if (!fs.existsSync(target) || !fs.existsSync(path.join(target, "db", "data.sqlite"))) {
    const legacy = process.platform === "win32"
      ? path.join(process.env.APPDATA || "", "9router")
      : path.join(os.homedir(), ".9router");
    if (fs.existsSync(path.join(legacy, "db", "data.sqlite"))) {
      try {
      ...
      if (platform === "win32") {
        ...
        try {
          ...
          lines.forEach(line => {
            ...
            const isAppProcess =
              (cmd.includes("node") && (cmd.includes("rcrouter") || cmd.includes("9router")) && (cmd.includes("cli.js") || cmd.includes("rcrouter") || cmd.includes("9router")))
      ...
      while (true) {
        ...
        if (choice === "update") {
          ...
          if (process.platform === "darwin") {
            ...
            console.log(`🔔 RcRouter is running in tray (PID: ${process.pid})`);
            console.log(`   Server: http://${displayHost}:${port}`);
          ...
          console.log(`🔔 RcRouter is now running in background (PID: ${bgProcess.pid})`);
          console.log(`   Server: http://${displayHost}:${port}`);
          console.log(`\n💡 You can close this terminal. Right-click tray icon to quit.\n`);
