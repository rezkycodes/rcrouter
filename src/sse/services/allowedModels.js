...
export async function fetchModelsFetcherIds(providerId, providerInfo) {
  ...
  try {
    ...
    const ids = rawModels.reduce((acc, m) => {
...
async function buildAllModelEntries(kindFilter, combos, customModels, modelAliases, isDisabled, activeConnectionByProvider, dbAvailable, options = {}) {
  ...
  if (!dbAvailable) {
    ...
    const freeProviders = Object.entries(FREE_PROVIDERS).filter(([providerId]) =>
    ...
    const freeResults = await Promise.allSettled(
      freeProviders.map(([providerId, providerInfo]) =>
...
export async function buildModelsList(kindFilter, options = {}) {
  ...
  const entries = await buildAllModelEntries(kindFilter, combos, customModels, modelAliases, isDisabled, activeConnectionByProvider, dbAvailable, options);
  ...
  for (const entry of entries) {
    ...
    if (Number.isFinite(entry.context_length)) model.context_length = entry.context_length;
    if (Number.isFinite(entry.max_completion_tokens)) model.max_completion_tokens = entry.max_completion_tokens;
...
export async function getAllowedModelIds() {
  ...
  const allIds = new Set(["auto", "auto/coding", "auto/fast", "auto/cheap", "auto/reasoning"]);
  for (const entry of entries) {
