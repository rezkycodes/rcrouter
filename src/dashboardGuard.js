...
let cachedCliTokens = null;
async function getCliTokens() {
  if (!cachedCliTokens) {
    const tokens = await Promise.all(CLI_TOKEN_SALTS.map((salt) => getConsistentMachineId(salt)));
    cachedCliTokens = new Set(tokens.filter(Boolean));
  }
  return cachedCliTokens;
}

async function hasValidCliToken(request) {
  const validTokens = await getCliTokens();
...
function isLoopbackPeer(request) {
  if (hasTrustedPeerHeaders(request)) {
    return isLoopbackHostname(request.headers.get("x-rc-real-ip") || request.headers.get("x-9r-real-ip"));
  }
...
export function isLocalRequest(request) {
  ...
  if (request.headers.get("x-rc-via-proxy") || request.headers.get("x-9r-via-proxy")) return false;
