...
const has9RouterConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "RcRouter" || entry.name === "9Router");
};

const get9RouterEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "RcRouter" || entry.name === "9Router") || null;
};
...
export async function POST(request) {
  try {
    ...
    const idx = config.findIndex((e) => e.name === "RcRouter" || e.name === "9Router");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "RcRouter" && e.name !== "9Router");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "RcRouter removed from Copilot config",
    });
  } catch (error) {
