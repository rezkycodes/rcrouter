...
const has9RouterConfig = (settings) => {
  ...
  return settings.customModels.some(m => m.id?.startsWith("custom:RcRouter") || m.id?.startsWith("custom:9Router"));
};
...
export async function POST(request) {
  try {
    ...
    // Remove all existing RcRouter and 9Router configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:RcRouter") && !m.id?.startsWith("custom:9Router"));
...
export async function DELETE() {
  try {
    ...
    // Remove RcRouter and legacy 9Router customModels
    if (settings.customModels) {
      settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:RcRouter") && !m.id?.startsWith("custom:9Router"));
    ...
    return NextResponse.json({
      success: true,
      message: "RcRouter custom models removed from Factory Droid",
    });
  } catch (error) {
