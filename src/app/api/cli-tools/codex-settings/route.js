...
const has9RouterConfig = (config) => {
  ...
  return config.includes("model_provider = \"RcRoute\"") ||
         config.includes("[model_providers.RcRoute]") ||
...
export async function POST(request) {
  try {
    ...
    parsed.model = model;
    parsed.model_provider = "RcRoute";

    // Update or create RcRoute provider section
    const normalizedBaseUrl = baseUrl.endsWith("/v1") ? baseUrl : `${baseUrl}/v1`;
    setNestedSection(parsed, "model_providers.RcRoute", {
      name: "RcRouter",
...
export async function DELETE() {
  try {
    ...
    if (parsed.model_provider === "RcRoute" || parsed.model_provider === "9router") {
      delete parsed.model;
      delete parsed.model_provider;
    }

    // Remove RcRoute and legacy 9router provider sections
    deleteNestedSection(parsed, "model_providers.RcRoute");
    ...
    return NextResponse.json({
      success: true,
      message: "RcRouter settings removed successfully",
    });
  } catch (error) {
