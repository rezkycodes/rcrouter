...
export default function DroidToolCard({
  ...
  const currentBaseUrl = droidStatus?.settings?.customModels?.find((m) => m.id?.startsWith("custom:RcRouter") || m.id?.startsWith("custom:9Router"))?.baseUrl || "";
  ...
  const getConfigStatus = () => {
    ...
    const currentConfig = droidStatus.settings?.customModels?.find(m => m.id?.startsWith("custom:RcRouter") || m.id?.startsWith("custom:9Router"));
  ...
  useEffect(() => {
    if (droidStatus?.installed && !hasInitializedModel.current) {
      ...
      const existingModels = (droidStatus.settings?.customModels || [])
        .filter(m => m.id?.startsWith("custom:RcRouter") || m.id?.startsWith("custom:9Router"))
      ...
      if (existingModels.length > 0) {
        ...
        const legacy = droidStatus.settings?.customModels?.find(m => m.id === "custom:RcRouter-0" || m.id === "custom:9Router-0");
        if (legacy?.model) {
          ...
          {!checkingDroid && droidStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {droidStatus?.settings?.customModels?.find(m => m.id?.startsWith("custom:RcRouter") || m.id?.startsWith("custom:9Router"))?.baseUrl && (
