...
export default function CombosPage() {
  ...
  const [autoComboConfig, setAutoComboConfig] = useState({});
  ...
  const fetchData = async () => {
    try {
      ...
      setAutoComboConfig(settingsData.autoComboConfig || {});
  ...
  const handleSaveAutoCombo = async (next) => {
    setAutoComboConfig(next);
    setSavingAuto(true);
    try {
      await fetch("/api/settings", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ autoComboConfig: next }),
      });
      setAutoSaved(true);
      setTimeout(() => setAutoSaved(false), 3000);
    } catch (error) {
      console.log("Error updating auto-combo config:", error);
    } finally {
      setSavingAuto(false);
    }
  };
    ...
    <div className="flex min-w-0 flex-col gap-6 px-1 sm:px-0">
      ...
      */}
      ...
      />
        ...
        <AutoComboSection
          autoComboConfig={autoComboConfig}
          onSave={handleSaveAutoCombo}
          combos={combos}
          saving={savingAuto}
          saved={autoSaved}
          copied={copied}
...
const STRATEGY_OPTIONS = [
  ...
  { value: "context-relay", label: "Context-Relay — cache-aware session affinity" },
  { value: "p2c", label: "P2C — power of two choices" },
  { value: "reset-aware", label: "Reset-Aware — prioritize expiring quota" },
...
const AUTO_ROUTES = [
  { key: "auto", name: "auto", label: "auto (General / Balanced)", desc: "Dynamically balances requests across all active healthy providers.", defaultStrategy: "context-relay" },
  { key: "auto/coding", name: "auto/coding", label: "auto/coding (Coding Tasks)", desc: "Directs coding requests to Claude, GLM, DeepSeek with prompt cache affinity (Context-Relay).", defaultStrategy: "context-relay" },
  { key: "auto/fast", name: "auto/fast", label: "auto/fast (Low Latency)", desc: "Dispatches to ultra-fast models (Groq, Cerebras, Flash) using Power-of-Two-Choices load balancing.", defaultStrategy: "p2c" },
  { key: "auto/cheap", name: "auto/cheap", label: "auto/cheap (Free & Cheap)", desc: "Prioritizes 100% free tiers (Kiro, OpenCode, GLM Flash) and expiring monthly quotas.", defaultStrategy: "reset-aware" },
  { key: "auto/reasoning", name: "auto/reasoning", label: "auto/reasoning (Thinking Models)", desc: "Directs deep reasoning to Kimi, DeepSeek R1, or thinking-enabled models with session continuity.", defaultStrategy: "context-relay" },
];
function AutoComboSection({ autoComboConfig = {}, onSave, combos = [], saving, saved, copied, onCopy, activeProviders = [] }) {
  const [localConfig, setLocalConfig] = useState(autoComboConfig || {});
  const [selectingRouteKey, setSelectingRouteKey] = useState(null);
  const [modelAliases, setModelAliases] = useState({});

  useEffect(() => {
    setLocalConfig(autoComboConfig || {});
  }, [autoComboConfig]);
  ...
  const updateRoute = (key, field, value) => {
    setLocalConfig((prev) => ({
      ...prev,
      [key]: {
        ...(prev[key] || {}),
  ...
  const handleAddModel = (routeKey, model) => {
    const prev = Array.isArray(localConfig[routeKey]?.customModels) ? localConfig[routeKey].customModels : [];
    if (!prev.includes(model.value)) {
      updateRoute(routeKey, "customModels", [...prev, model.value]);
    }
  };

  const handleDeselectModel = (routeKey, model) => {
    const prev = Array.isArray(localConfig[routeKey]?.customModels) ? localConfig[routeKey].customModels : [];
    updateRoute(routeKey, "customModels", prev.filter((m) => m !== model.value));
  };

  const handleRemoveModel = (routeKey, index) => {
    const prev = Array.isArray(localConfig[routeKey]?.customModels) ? localConfig[routeKey].customModels : [];
    updateRoute(routeKey, "customModels", prev.filter((_, i) => i !== index));
  };

  const handleMoveUp = (routeKey, index) => {
    if (index === 0) return;
    const prev = [...(Array.isArray(localConfig[routeKey]?.customModels) ? localConfig[routeKey].customModels : [])];
    [prev[index - 1], prev[index]] = [prev[index], prev[index - 1]];
    updateRoute(routeKey, "customModels", prev);
  };

  const handleMoveDown = (routeKey, index) => {
    const prev = [...(Array.isArray(localConfig[routeKey]?.customModels) ? localConfig[routeKey].customModels : [])];
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h3 className="text-base font-semibold text-text-main">Auto-Combo Routes</h3>
          <p className="text-xs text-text-muted mt-0.5">
            Configure how <code className="text-primary font-mono font-medium">auto/*</code> model routes behave when called by Claude Code, Cursor, or Codex.
          </p>
        </div>
        <Button
        ...
        >
          {saving ? "Saving..." : saved ? "Settings Saved ✓" : "Save Auto Settings"}
        </Button>
      </div>

      <div className="flex flex-col gap-3">
        {AUTO_ROUTES.map((route) => {
          const current = localConfig[route.key] || {};
          const mode = current.mode || "auto";
          const strategy = current.strategy || route.defaultStrategy;
          const linkedCombo = current.linkedCombo || "";
          const customModelsList = Array.isArray(current.customModels) ? current.customModels : [];

          return (
            <Card key={route.key} padding="sm" className="group">
              <div className="flex flex-col gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        >
                          <span className="material-symbols-outlined text-[14px]">
                            {copied === route.name ? "check" : "content_copy"}
                          </span>
                        </button>
                      </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 pt-2 border-t border-border/50">
                  ...
                  {mode === "combo" && (
                    <div>
                      ...
                      <Select
                        ...
                        value={linkedCombo}
                        onChange={(e) => updateRoute(route.key, "linkedCombo", e.target.value)}
                        selectClassName="py-1 text-xs"
                      />
                    </div>
                  )}
                  ...
                  {mode === "custom" && (
                    <div className="sm:col-span-2 lg:col-span-3 pt-1">
                      <label className="text-xs font-medium text-text-main mb-1.5 block">
                        Models ({customModelsList.length})
                      </label>

                      {customModelsList.length === 0 ? (
                        <div className="text-center py-6 border border-dashed border-black/15 dark:border-white/15 rounded-lg bg-black/[0.01] dark:bg-white/[0.01]">
                          <span className="material-symbols-outlined text-text-muted text-2xl mb-1">layers</span>
                          <p className="text-xs text-text-muted">No models added yet</p>
                        </div>
                      ) : (
                        <div className="flex max-h-[300px] min-w-0 flex-col gap-1.5 overflow-y-auto">
                          {customModelsList.map((model, idx) => (
                            <div
                              key={`${model}-${idx}`}
                              className="flex items-center gap-2 px-3 py-2 bg-surface rounded-lg border border-border text-xs"
                            >
                              <span className="material-symbols-outlined text-text-muted text-[16px] select-none">drag_indicator</span>
                              <span className="text-text-muted text-xs font-mono w-4">{idx + 1}</span>
                              <div className="flex-1 min-w-0 px-3 py-1.5 bg-surface-hover rounded border border-border/80 font-mono text-xs text-text-main truncate">
                                {model}
                              </div>
                              <div className="flex items-center gap-1 shrink-0">
                                <button
                                  type="button"
                                  disabled={idx === 0}
                                  onClick={() => handleMoveUp(route.key, idx)}
                                  className="p-1 text-text-muted hover:text-text-main disabled:opacity-20 disabled:cursor-not-allowed transition-colors"
                                  title="Move up"
                                >
                                ...
                                <button
                                  type="button"
                                  disabled={idx === customModelsList.length - 1}
                                  onClick={() => handleMoveDown(route.key, idx)}
                                  className="p-1 text-text-muted hover:text-text-main disabled:opacity-20 disabled:cursor-not-allowed transition-colors"
                                  title="Move down"
                                >
                                ...
                                <button
                                  type="button"
                                  onClick={() => handleRemoveModel(route.key, idx)}
                      ...
                      <button
                        type="button"
                        onClick={() => setSelectingRouteKey(route.key)}
                        className="w-full mt-2 py-2 border border-dashed border-primary/40 rounded-lg text-xs text-primary font-medium hover:text-primary hover:border-primary transition-colors flex items-center justify-center gap-1.5"
                      >
                        <span className="material-symbols-outlined text-[16px]">add</span>
                        Add Model
                      </button>
      ...
      {selectingRouteKey && (
        <ModelSelectModal
          isOpen={!!selectingRouteKey}
          onClose={() => setSelectingRouteKey(null)}
          onSelect={(m) => handleAddModel(selectingRouteKey, m)}
          onDeselect={(m) => handleDeselectModel(selectingRouteKey, m)}
          activeProviders={activeProviders}
          modelAliases={modelAliases}
          title={`Add Model to ${selectingRouteKey}`}
          addedModelValues={Array.isArray(localConfig[selectingRouteKey]?.customModels) ? localConfig[selectingRouteKey].customModels : []}
          closeOnSelect={false}
        />
      )}
    </div>
