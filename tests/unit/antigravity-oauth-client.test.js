...
process.env.RCROUTER_ANTIGRAVITY_OAUTH_CLIENT_ID = EXPECTED.clientId;
process.env.RCROUTER_ANTIGRAVITY_OAUTH_CLIENT_SECRET = EXPECTED.clientSecret;
process.env.RCROUTER_GOOGLE_OAUTH_CLIENT_ID = GOOGLE.clientId;
process.env.RCROUTER_GOOGLE_OAUTH_CLIENT_SECRET = GOOGLE.clientSecret;
...
describe("antigravity oauth client (deduped)", () => {
  ...
  it("src oauth.js imports shared client + keeps full shape", async () => {
    ...
    expect(src).not.toContain(EXPECTED.clientSecret);
    expect(src).not.toContain(GOOGLE.clientSecret);
  });
});
