...
describe("Antigravity quota-aware routing", () => {
  it("prefers the pinned account but falls back when it is excluded", async () => {
    mocks.getProviderConnections.mockResolvedValue([
      { id: "ag-a", email: "a@example.com", isActive: true },
      { id: "ag-b", email: "b@example.com", isActive: true },
    ]);

    await expect(getProviderCredentials("antigravity", new Set(), MODEL, {
      preferredConnectionId: "ag-b",
    })).resolves.toMatchObject({ connectionId: "ag-b" });
    await expect(getProviderCredentials("antigravity", new Set(["ag-b"]), MODEL, {
      preferredConnectionId: "ag-b",
    })).resolves.toMatchObject({ connectionId: "ag-a" });
  });

  it("falls back when the pinned account is no longer active", async () => {
    mocks.getProviderConnections.mockResolvedValue([
      { id: "ag-a", email: "a@example.com", isActive: true },
    ]);
    ...
    expect(mocks.getProviderConnections).toHaveBeenCalledWith({ provider: "antigravity", isActive: true });
  });
