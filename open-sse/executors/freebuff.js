...
const FREE_ROOT_AGENT_BY_MODEL = {
  ...
  "deepseek/deepseek-v4.1-flash": "base3-free-deepseek-flash",
  ...
  "google/gemini-3.1-flash-lite": "base3-free-gemini-3-8-flash",
  ...
  "moonshot/kimi-k2.6": "base3-free-kimi-k3-eco",
};
...
function classifySessionGate(code, message, currentModel) {
  ...
  if (code === "session_model_mismatch") {
    ...
    if (msg.includes("country") || msg.includes("region") || msg.includes("ip tier") || msg.includes("unsupported region")) {
...
export function normalizeUpstreamModel(model) {
  if (model === "upstage/solar-pro-4") return "upstage/solar-pro4";
  if (model === "deepseek/deepseek-v4.1-flash") return "deepseek/deepseek-v4-flash";
  if (model === "moonshot/kimi-k2.6") return "crof/kimi-k3-eco";
  return model;
}
...
async function endSession(token, instanceId, proxyOptions) {
  if (!instanceId) {
    try {
      const getRes = await fetchWithNetworkRetry(`${sessionOrigin()}${SESSION_PATH}`, {
        method: "GET",
        headers: {
...
async function requestSession(token, model, proxyOptions) {
  ...
  if (!response.ok) {
    if (response.status === 409 && data?.status === "model_locked") {
      const ended = await endSession(token, null, proxyOptions);
      if (ended) {
        for (const [k] of sessionCache) {
          if (k.startsWith(`${token}::`)) sessionCache.delete(k);
        }
        const retryRes = await fetchWithNetworkRetry(`${sessionOrigin()}${SESSION_PATH}`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
        ...
        if (retryRes.ok) {
          ...
          if (retryData?.status === "active") {
            const parsedExp = Date.parse(retryData.expiresAt || "");
            const entry = {
              instanceId: retryData.instanceId,
            ...
            return { instanceId: retryData.instanceId, status: "active" };
          }
          if (retryData?.status === "none") {
...
export class FreebuffExecutor extends BaseExecutor {
  ...
  async execute({ model: rawModel, body, stream, credentials, signal, log, proxyOptions = null }) {
    const model = normalizeUpstreamModel(rawModel);
    body.model = model;
