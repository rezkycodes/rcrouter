...
// would trip the breaker after accounts get rate-limited, blocking all
...
class CircuitBreaker {
  constructor(name, options = {}) {
    ...
    this.failureWindowMs = options.failureWindowMs || 0; // 0 = cumulative
  ...
  canExecute() {
    ...
    if (this.state === STATE.OPEN) {
      ...
      const multiplier = Math.min(
        Math.pow(2, Math.floor(this.openProbeCycles / this.backoffEscalationCount)),
