      ...
      for (const line of lines) {
        ...
        if (mode === STREAM_MODE.PASSTHROUGH) {
          ...
          if (trimmed.startsWith("data:") && trimmed.slice(5).trim() !== "[DONE]") {
            try {
              ...
              // Reasoning content sanitizer: if thinking mode is none/off or client specified reasoning: false,
              // strip reasoning_content and reasoning from deltas so it does not leak to client.
              if (shouldStripReasoning && parsed.choices) {
                for (const choice of parsed.choices) {
                  if (choice.delta) {
                  ...
                  if (choice.provider_specific_fields) {
                    delete choice.provider_specific_fields.reasoning_content;
                    delete choice.provider_specific_fields.reasoning;
                    fieldsInjected = true;
                  }
                }
              }
              ...
              if (isFinishChunk && isKimiModel && kimiToolCalls && !kimiToolCallsEmitted) {
                parsed.choices[0].delta = { ...parsed.choices[0].delta, tool_calls: kimiToolCalls };
        ...
        if (translated?.length > 0) {
          for (const item of translated) {
            ...
            if (shouldStripReasoning && item.choices) {
            ...
            if (isKimiModel && normalizeKimiToolCalls && isFinishChunk && kimiToolCalls && !kimiToolCallsEmitted) {
              if (item.choices?.[0]) {
                item.choices[0].delta = { ...item.choices[0].delta, tool_calls: kimiToolCalls };
...
export function createSSETransformStreamWithLogger(targetFormat, sourceFormat, provider = null, reqLogger = null, toolNameMap = null, model = null, connectionId = null, body = null, onStreamComplete = null, apiKey = null, customToolNames = null, credentials = null, normalizeKimiToolCalls = null, providerThinking = null) {
  if (typeof customToolNames === "function") {
    normalizeKimiToolCalls = customToolNames;
    customToolNames = null;
  }
...
export function createPassthroughStreamWithLogger(provider = null, reqLogger = null, model = null, connectionId = null, body = null, onStreamComplete = null, apiKey = null, normalizeKimiToolCalls = null, providerThinking = null) {
  return createSSEStream({
